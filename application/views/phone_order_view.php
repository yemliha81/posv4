						...
						<div style="">
							...
							<?php if( $disc == '1' ){ ?>
								<div class="row" style="background:#f7f7f7; margin:0 1px;white-space:nowrap;">
									<div class="clearfix"></div>
									<div class="col-xs-12 rightPrint hd" style="padding-top:5px;">
										<span class="pull-left xbb">İndirim :</span>
							...
							<div class="row" style="background:#fff; margin:0 1px;">
								<div class="clearfix"></div>
									<div class="col-xs-12 rightPrint hd" style="padding-top:5px;">
										<span class="pull-left xbb">Kalan :</span>
...
<?php include('includes/footer-order2.php');?>
  ...
  //location.reload();
	...
	function askForExit(){
		...
		if(order_content != ''){
			...
			/* swal({
			  title: '',
			  text: "Kaydetmeden çıkmak istediğinizden emin misiniz?",
			  type: 'warning',
			  showCancelButton: true,
			  confirmButtonColor: '#5b5351',
			  cancelButtonColor: '#d33',
	...
	text += '<tr class="'+pr+' '+hd+'">\
		...
		$(".addCustomer").click(function(){
			...
			if(full_name != ''){
				...
				swal('','İsim girilmesi zorunludur!','warning');
			}
		});
					...
					success : function(response){
						if(response != 'noresults'){
							...
							for( var key in default_data ) {
								innerHtml+= "<div><a href='javascript:;' phone='"+default_data[key].phone+"' name='"+default_data[key].full_name+"' custId='"+default_data[key].customer_id+"' class='selectCustomer' addresses='"+default_data[key].addresses+"'>";
	...
	$(document).on("click", ".selectCustomer", function(){
		...
		addrss += '<select name="address_id" class="form-control select2">';
