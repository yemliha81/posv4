	...
	<div class="left2">
		<div class="top">
			...
			<div>
				...
				<div class="row">
					<div class="col-sm-6">
						<span class="pageTitle">Stok İşlemleri <i class="fa fa-chevron-right f18"></i> Stok Sayım Güncelleme</span>
					</div>
					<div class="col-sm-6">
		...
		<div class="mainContainer"style="padding:0 15px;">
			<div class="whiteZone f13" >
				<div class="row">
					<div class="col-xs-12">
					<form id="stockForm" action="<?php echo STOCK_ENTRY_UPDATE_POST;?>" >
					
						<div class="row pB15 mB15 borderB"> 
							<div class="col-sm-2">
								<input type="text" name="date1" class="form-control datepicker required" nn="Tarih" value="<?php echo $purchase['date1'];?>" placeholder="Tarih" required />
							</div>
							<div class="col-sm-6">
								<input type="text" class="form-control" name="description" placeholder="Açıklama" value="<?php echo $purchase['description'];?>"/>
							</div>
							<div class="col-sm-4">
								<div class="row">
						...
						<input type="hidden" name="record_id" value="<?php echo $purchase['record_id'];?>" />
						<div class="clearfix"></div>
										...
										<select name="depo_id" class="form-control depoSelect select2 required" nn="Depo" required>
											...
											<?php foreach($depos as $key => $val){ ?>
												<option value="<?php echo $val['depo_id'];?>"
													<?php if($purchase['depo_id'] == $val['depo_id']){ echo 'selected'; }?>
												>
							...
							<div class="tables">
								...
								<?php if(!empty($list)){ ?>
									...
									<?php $rand = uniqid();?>
										<div class="row copied pB15 mB15 borderB" row="<?php echo $rand;?>" style="margin-bottom:10px;">
											<div class="col-sm-3">
												<input type="text" name="pro_name[]" class="form-control xclass" readonly value="<?php echo $val['pro_name'];?>"/>
												<input type="hidden" name="pro_id[]" class="form-control xclass pr_id" value="<?php echo $val['pro_id'];?>"/>
											</div>
											<div class="col-sm-2 cc">
												<select name="unit[]" class="required select2 calc form-control unit_<?php echo $rand;?>" nn="Birim" rel="<?php echo $rand;?>" required >
													
													<?php if($val['unit'] == 'lt'){ ?>
														<option value="lt" multiply="1">Lt</option>
														<option value="cl" multiply="0.01">Cl</option>
													<?php }elseif($val['unit'] == 'kg'){ ?>
														<option value="kg" multiply="1">Kg</option>
														<option value="gr" multiply="0.001">Gr</option>
													<?php }elseif($val['unit'] == 'adet'){ ?>
														<option value="adet" multiply="1">Adet</option>
													<?php } ?>
												</select>
											</div>
											<div class="col-sm-3 cc">
												<input type="number" step=".01" name="qty[]" class="xcopy calc form-control qty_<?php echo $rand;?>" relType="qty" rel="<?php echo $rand;?>" value="<?php echo $val['qty'];?>"/>
											</div>
											<div>
												<a href="javascript:;" class="delRow btn btn-sm btn-danger" rel="<?php echo $rand;?>">
							...
							<div>
								<div class="row">
									...
									<div class="col-sm-3">
										<input type="button" class="butonX1 w100 submForm " value="GÜNCELLE"/>
									</div>
								</div>
							</div>
						</form>
...
<script type="text/javascript">
	...
	$(".submForm").click(function(){
		...
		if( empty !== '1' ){
			...
			$.ajax({
				type : 'post',
				url : '<?php echo STOCK_COUNT_ENTRY_UPDATE_POST;?>',
				data : df,
	...
	$(".delR").click(function(){
		...
		swal({
		  ...
		  if (result.value) {
			...
			$.ajax({
				type : 'get',
				url : '<?php echo STOCK_COUNT_DELETE.$purchase['record_id'];?>',
				success : function(response){
					if(response == 'success'){
