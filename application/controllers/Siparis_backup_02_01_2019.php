...
class Siparis extends CI_Controller {
	...
	public function send_to_printer(){
		...
		$order = $this->db->select('total_price, discount_price, paid_price, rest_price, order_insert_time')
			->where('order_id', $post['order_id'])
			->get('orders_table')->row_array();
		...
		foreach( $arr['order_details'] as $key => $val ){
			...
			if($val['description'] != 'İptal'){
				...
				$ins['htmlBody'] .= '<div style="font-size:25px;">
										<div class="w60">'.$val['pro_name'].' '.$val['description'].'</div>
		...
		$ins['htmlBody'] .= '<div class="fBig">
									<div class="w60"><b>Tutar</b></div>
									<div class="w40 text-right">'.$arr['total_price'].' TL</div>
									<div class="clr"></div>
								</div>';
		...
		if( ($arr['discount_price'] > 0) OR ($arr['paid_price'] > 0) ){
			$ins['htmlBody'] .= '<div class="fBig">
										<div class="w60"><b>Toplam</b></div>
										<div class="w40 text-right">'.$arr['rest_price'].' TL</div>
										<div class="clr"></div>
									</div>';
		}
		$ins['htmlBody'] .= '</div>';
			...
			$this->db->delete('order_details_table', array('order_id' => $order_id));
				...
				//$this->db->truncate("aaa_print_table");	
					...
					foreach( $kitchen as $key => $val ){
						...
						$ins[$key]['htmlBody'] .= '<div class="text-left" style="font-size:22px;"><b>Masa No :</b> <span class="f50">'.$this->getTableName($post['table_id']).'</span></div>';
						$ins[$key]['htmlBody'] .= '<div class="text-left" style="font-size:22px;"><b>T. Saat :</b> '.date('d.m.Y H:i', time()).'</div>';
						$ins[$key]['htmlBody'] .= '<div class="text-left" style="font-size:22px;"><b>Garson  :</b> '.$_SESSION['user_name'].'</div>';
