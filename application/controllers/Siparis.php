...
class Siparis extends CI_Controller {
	...
	public function send_to_printer(){
		...
		if($settings['adisyon_group'] == '1'){
			$arr['order_details'] = $this->db->select('pro_id, porsion_id, pro_name, SUM(qty) qty , SUM(total_price) price, description')
				->where('order_id', $post['order_id'])
				->where('total_price > 0')
				->order_by('id', 'ASC')
				->group_by('porsion_id')
		...
		if(empty($settings['adisyon_group'])){
			$arr['order_details'] = $this->db->select('pro_id,pro_name, qty, price, description')
				->where('order_id', $post['order_id'])
				->where('total_price > 0')
				->order_by('id', 'ASC')
		...
		$row[] = array('type' => 'header', 'title' => $arr['user_name']);
		$row[] = array('type' => 'line', 'title' => '');
		$row[] = array('type' => 'productCell', 'cell1' => '', 'cell2' => 'Adet', 'cell3' => 'Tutar');
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		
		foreach( $arr['order_details'] as $key => $val ){
			if($val['description'] != 'İptal'){
				//$row[] = array('type' => 'tableCell', 'cell1' => $val['pro_name'].' '.$val['description'], 'cell2' => $val['qty'],'cell3' => '',  'cell4' => number_format(($val['price']),2));
				$row[] = array('type' => 'productCell', 'cell1' => $val['pro_name'].' '.str_replace('FD', '', $val['description']), 'cell2' => $val['qty'], 'cell3' => number_format(($val['price']),2));
	...
	public function send_to_printer_receipt(){
		...
		$row[] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
		$row[] = array('type' => 'line', 'title' => '');
		$row[] = array('type' => 'tableCell', 'cell1' => 'Adet', 'cell2' => '','cell3' => '',  'cell4' => 'Tutar');
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
			...
			$this->db->delete('order_details_table', array('order_id' => $order_id));
				...
				foreach($post['pro_id'] as $key => $val){
					...
					if($post['printed'][$key] == '0'){
						if (strpos($post['desc2'][$key], '(Beklet)') == false) {
				...
				//$this->db->truncate("aaa_print_table");	
					...
					foreach( $kitchen as $key => $val ){
						...
						foreach($val as $vv){
							...
							$row[$key][] = array('type' => 'cancel', 'title' => $vv['pro_name'].' X '.$vv['qty'] );
						}
	...
	public function calling($customer_id){
		...
		if(empty($data['customer'])){
			$data['new_number'] = $customer_id;
		}
		
		 // Kayıtlı Müşteri...
		$this->load->view('calling_details_view', $data);
	...
	public function get_category_reports($fdate, $ldate){
		...
		->group_by('order_details_table.pro_id')
		->order_by('c', 'DESC')
		->where('order_details_table.price > 0')
		->where('order_insert_time >=', $fdate)
		->where('order_insert_time <=', $ldate)
		//->limit(10)
		->get('order_details_table')->result_array();
	...
	public function cancelx_details(){
		...
		$data['list'] = $this->db->select('*, user_log_report.last_price as last_pp, user_log_report.id as log_id')
	...
	public function user_auth_post($ut_id=NULL){
		$post = $this->input->post();
		//debug($post);
		if($post['ut_id'] > '0'){
