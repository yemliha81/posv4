...
class Siparis extends CI_Controller {
	...
	public function phone_order(){
		...
		$this->load->view('phone_order_view', $data); 
	}
			...
			$this->db->delete('order_details_table', array('order_id' => $order_id));
				...
				foreach($post['pro_id'] as $key => $val){
					...
					if($post['printed'][$key] == '0'){
						$kitchenId = $this->findKitchen($post['pro_id'][$key]);
						$kitchen[$kitchenId][$key]['pro_id'] = $post['pro_id'][$key];
						$kitchen[$kitchenId][$key]['pro_name'] = $post['pro_name'][$key];
				...
				//$this->db->truncate("aaa_print_table");	
					...
					foreach( $kitchen as $key => $val ){
						...
						$ins[$key]['htmlBody'] .= '<html><body>';
						$ins[$key]['htmlBody'] .= '<table>';
						foreach($val as $vv){$ins[$key]['htmlBody'] .= '<tr><td>'.$vv['pro_name'].'</td><td>'.$vv['qty'].'</td></tr>';}
						$ins[$key]['htmlBody'] .= '</table>';
						$ins[$key]['htmlBody'] .= '</body></html>';
						$ins[$key]['insertTime'] = time();
							$this->db->insert('aaa_print_table',$ins[$key]); */
						$ins[$key]['printerName'] = $key; 
						$ins[$key]['htmlBody'] .= '<html><head>
													<style type="text/css">
													@media print{
													html, body{padding:0mm;margin:0mm;font-family:Arial;font-size:13px;}.tr{w border-top: 1px solid #000; padding:10px 0;}.w33{width:33%; float:left; display:inline-block;}.clr{clear:both;}
													}
													</style>
													<meta http-equiv="Content-Type" content="text/html;charset=UTF-8" />
												</head>';
						$ins[$key]['htmlBody'] .= '<body>';
						$ins[$key]['htmlBody'] .= '<div>';
						$ins[$key]['htmlBody'] .= '<div class="tr"><div class="w33"><b>Ürün</b></div><div class="w33"><b>Adet</b></div><div class="w33"><b>Fiyat</b></div><div class="clr"></div></div>';
						foreach($val as $vv){$ins[$key]['htmlBody'] .= '<div class="tr"><div class="w33">'.$vv['pro_name'].'</div><div class="w33">'.$vv['qty'].'</div><div class="w33"></div><div class="clr"></div></div>';}
						$ins[$key]['htmlBody'] .= '<div class="clr"></div>';
						$ins[$key]['htmlBody'] .= '</div>';
						$ins[$key]['htmlBody'] .= '</body>';
						$ins[$key]['htmlBody'] .= '</html>';
	...
	public function reload_orders($kid){
		...
		foreach($data['orders'] as $key => $val){
			...
			$data['orders'][$key]['order_details'] = $this->db->select('order_details_table.order_id,menu_cat_product_table.cat_id, order_details_table.id, order_details_table.pro_name,order_details_table.description, order_details_table.qty, order_details_table.ready')
				->join('menu_cat_product_table','order_details_table.pro_id = menu_cat_product_table.pro_id', 'left')
